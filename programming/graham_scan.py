...
def get_next_point(stack, points, next_point):
    ...
    last_vector = pygame.Vector2(points[last_point][0] - points[past_point][0], -points[last_point][1] + points[past_point][1])
    next_vector = pygame.Vector2(points[next_point][0] - points[last_point][0], -points[next_point][1] + points[last_point][1])

    cross_product = last_vector.x * next_vector.y - next_vector.x * last_vector.y
    logging.info(f"stack={stack} points={points} last_point={last_point} next_point={next_point} past_point={past_point} current_vector={last_vector} next_vector={next_vector} cross_product={cross_product}")
    if cross_product <= 0:
        stack.pop()
    else:
