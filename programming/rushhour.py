...
def car_cells(car) -> list[tuple[int, int]]:
    ...
    if car_type == HORIZONTAL_CAR:
        return [(car_x, car_y), (car_x + 1, car_y)]

    return [(car_x, car_y), (car_x, car_y + 1)]
    ...
    while True:
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if mode == "play":
                    cars = move_car(cars, event.pos, event.button, screen_size)
                elif mode == "edit":
                    if event.button == 2:
                        cars = remove_car(cars, event.pos, event.button, screen_size)
                    else:
                        cars = add_car(cars, event.pos, event.button, screen_size)
